...
import { adminCategoriesService } from '@/app/services/adminService'
import type { ApiCategoryOut } from '@/app/services/apiTypes'
import { ApiError } from '@/app/services/api'

type FormData = { name: string; slug: string; description: string }
const empty: FormData = { name: '', slug: '', description: '' }
...
export default function CategoriesPage() {
  const [items, setItems]     = useState<ApiCategoryOut[]>([])
  const [loading, setLoading] = useState(true)
  const [open, setOpen]       = useState(false)
  const [editing, setEditing] = useState<ApiCategoryOut | null>(null)
  ...
  async function load() {
    setLoading(true)
    try { setItems(await adminCategoriesService.list()) } finally { setLoading(false) }
  }
  useEffect(() => { load() }, [])

  function openNew() { setEditing(null); setForm(empty); setError(''); setOpen(true) }
  function openEdit(c: ApiCategoryOut) {
    setEditing(c)
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (editing) {
        await adminCategoriesService.update(editing.id, payload)
      } else {
  ...
  async function handleDelete(id: string, name: string) {
    ...
    try { await adminCategoriesService.remove(id); load() } catch { /* ignore */ }
  }
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-[#e8d5b0]">Categorias</h1>
          <p className="text-[#6b5c45] text-sm mt-1">{items.length} cadastradas</p>
        </div>
        <button
          onClick={openNew}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#2a2016]">
                ...
                <th className="text-left px-5 py-3 text-[#6b5c45] font-medium">Descrição</th>
            ...
            <tbody>
              {items.map(c => (
                <tr key={c.id} className="border-b border-[#1a140e] hover:bg-white/2 transition-colors">
                  <td className="px-5 py-3 text-[#e8d5b0]">{c.name}</td>
                  <td className="px-5 py-3 text-[#6b5c45] font-mono text-xs">{c.slug}</td>
                  <td className="px-5 py-3 text-[#9a8a74] max-w-xs truncate">{c.description ?? '—'}</td>
                  <td className="px-5 py-3 text-[#6b5c45] text-right">{c.books_count}</td>
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-2 justify-end">
                      <button onClick={() => openEdit(c)} className="text-[#6b5c45] hover:text-[#c8a96e] transition-colors">
                        <Pencil size={14} />
                      </button>
                      <button onClick={() => handleDelete(c.id, c.name)} className="text-[#6b5c45] hover:text-red-400 transition-colors">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </td>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-[#120e09] border border-[#2a2016] rounded-xl w-full max-w-md p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs text-[#6b5c45] mb-1">Descrição (opcional)</label>
                <textarea
