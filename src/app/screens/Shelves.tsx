...
import { BookOpen, CheckCircle, Clock, Plus, Grid3x3, List } from 'lucide-react';
            ...
            {shelves.map((shelf) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-xs opacity-80">({shelf.count})</span>
                </button>
              );
            })}
          </div>
            ...
            {readingBooks.map((book, i) => (
              ...
              >
                <div className="flex gap-4 mb-4">
                  <div className={`w-16 h-24 rounded-lg bg-gradient-to-br ${book.cover} flex-shrink-0`}></div>
                  <div className="flex-1">
                    <h3 className="text-sm font-medium mb-1">{book.title}</h3>
                    <p className="text-xs text-[#8a7e6e] mb-3">{book.author}</p>
                    <div className="flex items-center gap-2 text-xs text-[#8a7e6e]">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-3 mb-6">
              <div className="bg-[#1a1210]/50 border border-[#c8a96e]/20 rounded-xl p-4 text-center">
                <p className="text-2xl font-bold text-[#c8a96e] mb-1">47</p>
                <p className="text-xs text-[#8a7e6e]">Livros lidos</p>
              </div>
              <div className="bg-[#1a1210]/50 border border-[#c8a96e]/20 rounded-xl p-4 text-center">
                <p className="text-2xl font-bold text-[#c8a96e] mb-1">4.3</p>
                <p className="text-xs text-[#8a7e6e]">Nota média</p>
              </div>
              <div className="bg-[#1a1210]/50 border border-[#c8a96e]/20 rounded-xl p-4 text-center">
                <p className="text-2xl font-bold text-[#c8a96e] mb-1">12</p>
                <p className="text-xs text-[#8a7e6e]">Este ano</p>
              </div>
            </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                {readBooks.map((book, i) => (
                  <button key={i} className="text-left">
                    <div className={`w-full aspect-[2/3] rounded-lg bg-gradient-to-br ${book.cover} mb-2 hover:scale-105 transition-transform relative`}>
